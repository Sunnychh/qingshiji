(function () {
  ...
  document.getElementById("auth-form").addEventListener("submit", async function (event) {
    ...
    var email = new FormData(form).get("email").trim();
    var submit = form.querySelector("button[type=submit]");
    submit.disabled = true;
    submit.textContent = "正在发送…";
    document.getElementById("auth-status").textContent = "";
    var result = await dbClient.auth.signInWithOtp({
      email: email,
      options: { emailRedirectTo: window.location.origin + window.location.pathname },
    });
    submit.disabled = false;
    submit.textContent = "发送登录邮件";
    if (result.error) {
      console.error(result.error);
      document.getElementById("auth-status").textContent = "发送失败：" + result.error.message;
      return;
    }
    document.getElementById("auth-status").textContent = "邮件已发送，请点击邮件中的链接完成登录。";
  });
